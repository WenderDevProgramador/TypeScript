...
function addCrewMember(member, spaceship) {
    if (spaceship.crew.length >= spaceship.crewLimit) {
        alert(`${member} não pode ser adicionado o limite já foi atingido.`);
    }
    else {
        spaceship.crew.push(member);
        alert(`${member} foi adicionado à tripulação ${spaceship.name} com sucesso.`);
    }
}
function sendInMission(spaceship) {
    if (spaceship.inMission) {
        alert(`${spaceship.name} não pode ser enviada. Nave já esta em missão`);
    }
...
function firstMenuOption() {
    ...
    const pilot = prompt(`Qual é o nome do piloto da nave ${name} ?`);
    const crewLimit = Number(prompt(`Quantos tripulantes a ${name} suporta ?`));
    const confirmate = confirm(`Confirma o registro ?  
        Nave: ${name}
        Piloto: ${pilot}`);
    if (confirmate) {
